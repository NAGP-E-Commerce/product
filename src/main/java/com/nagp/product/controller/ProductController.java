...
public class ProductController {
	...
	public ResponseEntity<ProductDTO> getById(@PathVariable("id") 
	@NotBlank(message = "id must not be empty") String id) {
		Product product = productService.getByProductId(id);
		ProductDTO productDTO = ProductMapping.getProductToProductDTO(product);
	...
	public ResponseEntity<List<ProductDTO>> getAllProducts() {
		...
		for(Product product : products) {
			productDTOs.add(ProductMapping.getProductToProductDTO(product));
		}
		return ResponseEntity.status(HttpStatus.OK).body(productDTOs);
	}
	
	@RequestMapping(path = "product/code/{code}", method = RequestMethod.GET)
	@ApiOperation("Gets product by code")
	@ApiResponses(value = {@ApiResponse(code = 200, message = "OK", response = ProductDTO.class)})
	public ResponseEntity<ProductDTO> getByCode(@PathVariable("code") 
	...
	@RequestMapping(path = "product/special", method = RequestMethod.GET)
	...
	@RequestMapping(path = "product/bestseller", method = RequestMethod.GET)
	...
	@RequestMapping(path = "product/featured", method = RequestMethod.GET)
	...
	public ResponseEntity<Object> createProduct(@RequestBody ProductDTO productDTO) {
		...
		productDTO = ProductMapping.getProductToProductDTO(dbCopy);
		return ResponseEntity.status(HttpStatus.OK).body(productDTO);
	...
	@ApiResponses(value = {@ApiResponse(code = 200, message = "OK", response = ProductDTO.class)})
	public ResponseEntity<Boolean> deleteProductById(@PathVariable("id") 
	@NotBlank(message = "id must not be empty") String id) {
