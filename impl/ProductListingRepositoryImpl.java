...
public class ProductListingRepositoryImpl implements ProductListingRepository {
	...
	credentialsProvider.setCredentials(AuthScope.ANY,new UsernamePasswordCredentials("elastic", "2IWsOeVvLI43Olp8Mx1T8Wnq"));
	
	RestClientBuilder builder = RestClient.builder(new HttpHost("3637c5436bcd41c7b1f802d6a7b84fc4.us-central1.gcp.cloud.es.io", 9243, "https")).setHttpClientConfigCallback(httpClientBuilder -> httpClientBuilder.setDefaultCredentialsProvider(credentialsProvider));
	...
	public List<ProductDTO> findAllProductDataByCategoryCodeFromElastic(final String categoryCode, String url) {
		...
		searchRequest.indices("productmanage");
		final SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
		searchSourceBuilder.query(QueryBuilders.boolQuery().must(QueryBuilders.termQuery("categoryCode.keyword", categoryCode)));
		searchRequest.source(searchSourceBuilder);
		final List<Product> productList = new ArrayList<>();
